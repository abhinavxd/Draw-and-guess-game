...
const TopGameHeader = ({ gameId, gameStarted, currentWord }) => {
    return (
        <div className={styles.topGameBar}>
            <div>{gameId !== undefined ? (<div>{!gameStarted ? "To start drawing share this URL with your friend" : "Room URL"} : <a href={`${window.location.origin}/${gameId}`}>{process.env.REACT_APP_DOMAIN}/{gameId}</a></div>) : ("")} </div>
            {currentWord && <div>
